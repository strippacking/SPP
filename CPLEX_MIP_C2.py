...
if not os.path.exists('CPLEX_MIP_C2'):
    os.makedirs('CPLEX_MIP_C2')
...
def solve_strip_packing(widths, heights, strip_width, time_limit=1800):
    """Solve the Strip Packing Problem using CPLEX MIP with time limit"""
    ...
    for i in range(n):
        for j in range(i+1, n):
            # If two rectangles can't fit side by side, force vertical stacking
...
def display_solution(strip, rectangles, pos_circuits, instance_name):
    ...
    plt.savefig(f'CPLEX_MIP_C2/{instance_name}.png')
    plt.close()
...
if __name__ == "__main__":
    ...
    # Phần single instance mode
        ...
        try:
            ...
            upper_bound = min(sum(heights), calculate_first_fit_upper_bound(strip_width, rectangles))

            print(f"Solving 2D Strip Packing with CPLEX MIP for instance {instance_name}")
            ...
            if result:
                ...
                if best_positions:
                    display_solution((strip_width, optimal_height), rectangles, best_positions, instance_name)
