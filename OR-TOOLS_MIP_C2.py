...
if not os.path.exists('OR-TOOLS_MIP_C2'):
    os.makedirs('OR-TOOLS_MIP_C2')
...
def solve_strip_packing(strip_width, items, time_limit=1800):
    """
    Solves the strip packing problem with time limit.
    ...
    # Variables
    x = [solver.NumVar(0, strip_width - items[i][0], f'x_{i}') for i in range(n)]
    ...
    # 2. One pair of rectangles constraint (C2 - pair)
    if n >= 2:
        solver.Add(left[1][0] == 0)  # Rectangle 1 cannot be to the left of rectangle 0
        solver.Add(below[1][0] == 0) # Rectangle 1 cannot be below rectangle 0
    
    # Find max area rectangle for domain reduction
    max_area_idx = 0
    ...
    # 3. Domain Reduction constraint (C2 - domain)
    # Limit x position of largest rectangle to first half of strip
    solver.Add(x[max_area_idx] <= (strip_width - items[max_area_idx][0]) / 2)
    
    # 4. Same Rectangles constraint (C2 - same)
    for i in range(n):
        for j in range(i+1, n):
            if items[i][0] == items[j][0] and items[i][1] == items[j][1]:
                # If identical rectangles, fix relative positions to break symmetry
                solver.Add(left[j][i] == 0)  # j can't be left of i
                solver.Add(left[i][j] + below[j][i] >= 1)  # Either i left of j or j below i
...
def display_solution(strip_width, height, items, positions, instance_name):
    ...
    plt.savefig(f'OR-TOOLS_MIP_C2/{instance_name}.png')
    plt.close()
...
if __name__ == "__main__":
    ...
    if len(sys.argv) == 1:
        ...
        excel_file = 'OR-TOOLS_MIP_C2.xlsx'
        ...
        for instance_id in range(1, 42):
            ...
            command = f"./runlim --real-time-limit={TIMEOUT} python3 OR-TOOLS_MIP_C2.py {instance_id}"
    ...
    # Phần single instance mode
        ...
        try:
            ...
            upper_bound = min(sum(h for _, h in items), calculate_first_fit_upper_bound(strip_width, items))

            print(f"Solving 2D Strip Packing with OR-Tools MIP for instance {instance_name}")
            ...
            if solution:
                display_solution(strip_width, solution['height'], items, solution['positions'], instance_name)
